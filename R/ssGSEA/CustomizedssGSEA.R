...
CustomizedssGSEA <- function(expressionProfile, signatureList){
  ...
  ssGSEA.score <- sapply(signatureList, function(signatureSet){
    ...
    indicatorInsideGeneSet.matrix <- indicatorInsideGeneSetMatrix(superposition.rank = superposition.rank.exp, 
    ...
    signature.rank <- apply(signature.temp.rank, 2, function(r){rankByDesc(r, item.num = exp.m)})
