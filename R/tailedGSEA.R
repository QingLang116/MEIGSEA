...
tailedGSEA <- function(exp.profile, is.rawcount= TRUE, sample.group, case, control, sig.df, min.sz = 15, pvalueCutoff = 1, pAdjustMethod = "BH",
  ...
  if(is.rawcount){
    ...
    if(any(!is.integer(as.matrix(exp.profile)))) {stop("** This is not a integer profile ! ")}
    if(any(exp.profile < 0)) {stop("** This is not a non-negative profile ! ")}
  ...
  if(any(geneset.size < min.sz)){tmp.sig.df <- tmp.sig.df[-which(tmp.sig.df[,"setAnno"] %in% names(geneset.size)[which(geneset.size < min.sz)]),]}
  ...
  if(any(geneset.size < min.sz)){tmp.sig.df <- tmp.sig.df[-which(tmp.sig.df[,"label"] %in% names(geneset.size)[which(geneset.size < min.sz)]),]}
  
  if(nrow(tmp.sig.df) > 0){
    twotailedGSEA.res <- twotailedGSEA(log2FC = log2FC, sig.df = tmp.sig.df, min.sz = min.sz, pvalueCutoff = pvalueCutoff, pAdjustMethod = pAdjustMethod, is.fgsea = is.fgsea,
