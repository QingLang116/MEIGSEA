...
MEIGSEA <- function(raw.count.profile, is.rawcount = TRUE, gene.length = NULL, tpm.exp = NULL, mutationCalling.map, signature.df, 
	...
	original.res <- IntegratedGSEA(raw.count.profile = raw.count.profile, is.rawcount = is.rawcount, gene.length = gene.length, tpm.exp =  tpm.exp, mutationCalling.map = mutationCalling.map, 
	...
	if(cond.corr.anal){
		...
		sig.corscore.df <- CondCorrAnal(corscore.df, raw.count.profile = raw.count.profile, is.rawcount = is.rawcount, gene.length = gene.length, tpm.exp =  tpm.exp, mutationCalling.map = mutationCalling.map, 
