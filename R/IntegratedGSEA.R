...
IntegratedGSEA <- function(raw.count.profile, is.rawcount = TRUE, gene.length = NULL, tpm.exp = NULL, mutationCalling.map, signature.df, 
  ...
  if(all(table(genesetMeancor.res$df$setAnno) < min.sz)){
    ...
    check.size <- sapply(check.size, function(x) {return(ifelse(all(x < min.sz), T, F))}) 
    if(length(which(check.size)) > 0){
      ...
      refine.restore.signature.df <- data.frame(gene = restore.signature.df$Gene, setAnno = restore.signature.df$setAnno, 
  ...
  GSEAbased.cor.list <- lapply(rownames(mutationCalling.map), function(g){
    ...
    #'######
    one_twoGSEA.res <- tailedGSEA(exp.profile = raw.count.profile, is.rawcount = is.rawcount, sample.group = sample.group, case = case, control = control, 
    ...
    if(any(check.size < min.sz)){genesetMeancor.res$list <- genesetMeancor.res$list[-which(check.size < min.sz)]}
    print(paste0("*** length of signature list is ", length(genesetMeancor.res$list)))
    
    if(length(genesetMeancor.res$list) > 0) {
      ssGSEA.wilcox.res <- scoreDiffAssess(exp.profile = TPM.profile, sig.list = genesetMeancor.res$list, min.sz = min.sz,
  ...
  corscore.list <- lapply(GSEAbased.cor.list, function(corRes.oneGene){
    corRes.oneSig.list <- lapply(unique(corRes.oneGene$comb.relations.df$signature), function(one.sig){
      ...
      tmp.res <- data.frame(signature = corRes.oneSig$signature[1], 
