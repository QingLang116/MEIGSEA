...
CondCorrAnal <- function(corscore.df, raw.count.profile, is.rawcount = TRUE, gene.length = NULL, tpm.exp = NULL, mutationCalling.map, 
	...
	if(!is.null(remain.genepairs)){
		...
		sig.corscore.df <- getIndAssociations(remain.genepairs, sig.corscore.df, raw.count.profile, mutationCalling.map, TPM.profile = TPM.profile , sig.cutoff= sig.cutoff, signature.df= signature.df, 
